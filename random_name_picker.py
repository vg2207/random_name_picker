...
if user_input_excel is not None:
    if user_input_excel.name.endswith('.csv'):
        ...
        # st.sidebar.write(df)
        lower_col = []
        upper_col = ['ID Karyawan (tuliskan dengan lengkap dan kapital, contoh: LTL002965)', 'Nama Lengkap']
        df = lowerify_and_upperify_cols(df, lower_col, upper_col)
        df = remove_duplicate(df, drop_col='ID Karyawan (tuliskan dengan lengkap dan kapital, contoh: LTL002965)')
        df = df.loc[(df['Level'] != 'BOD')].reset_index(drop=True)
        df.rename(columns = {"ID Karyawan (tuliskan dengan lengkap dan kapital, contoh: LTL002965)":"ID Karyawan"}, inplace = True)
                ...
                if st.session_state.count <= len(df2) :
                    ...
                    if st.session_state.count <= 2 :
                        ...
                        for col in row1:
                            ...
                            tile.markdown(f"<h5 style='text-align: center;'>{str(pd.DataFrame(data_to_show).loc[(m+1),'ID Karyawan'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Nama Lengkap'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Company'])}</h5>", unsafe_allow_html=True)
                            m+=1
                        for col in row2:
                            tile = col.container(border=True)
                            tile.markdown(f"<h5 style='text-align: center;'>{str(pd.DataFrame(data_to_show).loc[(m+1),'ID Karyawan'])}<br>
                        ...
                        for col in row1:
                            tile = col.container(border=True)
                            tile.markdown(f"<h2 style='text-align: center;'>{str(pd.DataFrame(data_to_show).loc[(m+1),'ID Karyawan'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Nama Lengkap'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Company'])}</h2>", unsafe_allow_html=True)
                            m+=1
                        ...
                        for col in row1:
                            tile = col.container(border=True)
                            tile.markdown(f"<h1 style='text-align: center;'>{str(pd.DataFrame(data_to_show).loc[(m+1),'ID Karyawan'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Nama Lengkap'])}<br>
                                {str(pd.DataFrame(data_to_show).loc[(m+1),'Company'])}</h1>", unsafe_allow_html=True)
                            m+=1
...
#                        'Hydrogen Tumbler by LSA', 'Massage Pillow',
#                        'Blender Juicer', 'Free Scaling Voucher - Audy Dental',
#                        'Container by LNK', 'Cosmos Mixer by LNK',
#                        'Oxone Tools by LNK', 'Voucher Indomaret Rp 100.000',
